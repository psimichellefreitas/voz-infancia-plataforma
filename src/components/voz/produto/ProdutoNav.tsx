...
const ITEMS = [
  ...
  { label: "Buscar", to: "/voz-protetora/busca" },
...
export function ProdutoNav() {
  ...
  return (
    <header className="print:hidden fixed inset-x-0 top-0 z-50 border-b border-border bg-background/95 backdrop-blur-md">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-5 py-3 sm:px-8">
        ...
        <nav className="hidden items-center gap-5 xl:flex">
          ...
          <Link
            ...
            search={previewSearch}
            className="relative grid h-8 w-8 place-items-center rounded-full text-foreground/70 transition-colors hover:text-primary"
            activeProps={{ className: "text-primary" }}
        ...
        <div className="flex items-center gap-2 xl:hidden">
          <Link
            to="/voz-protetora/notificacoes"
            search={previewSearch}
            className="relative grid h-10 w-10 place-items-center rounded-[10px] text-primary"
            aria-label="Notificações"
          >
            <Bell className="h-5 w-5" />
            {naoLidas > 0 && (
